...
<div id="modulesContainer" class="container">
    <!-- Module-Vendor Group -->
    <div class="module-group border p-3 mb-3">
        <div class="row">
            <!-- Module select -->
            <div class="col-lg-4 mb-3">
                ...
                <label class="form-element-label">Module</label>
            </div>

            <!-- Vendors Container -->
            <div class="col-lg-6 mb-3 vendor-container">
                <div class="vendor-row d-flex mb-2">
                    <select name="primary_vendor[0][]" class="form-control vendor-select me-2 primary-vendor-select primary-vendor-select">
                        <option value="">Select Vendor</option>
                        <!-- Vendor options -->
                    </select>
                    <button type="button" class="btn btn-danger remove-vendor">Remove Vendor</button>
                </div>
                <button type="button" class="btn btn-secondary add-vendor">Add Vendor</button>
            </div>
...
<div class="">
    <!-- <button class="btn btn-outline-primary plus-btn" type="button"><i class="bi bi-plus-lg"></i> Add More</button> -->
     <!-- Add Module button -->
<button type="button" class="btn btn-primary" id="addModuleGroup">Add Module + Vendor Group</button>
</div>
<hr>
...
$(document).ready(function () {
    ...
    // Add Module-Vendor group
    $('#addModuleGroup').click(function () {
        let newGroup = $('.module-group').first().clone();
        newGroup.find('select').val('');
        newGroup.find('.vendor-container').html(`
            <div class="vendor-row d-flex mb-2">
                <select name="primary_vendor[${moduleIndex}][]" class="form-control vendor-select me-2 primary-vendor-select">
                    <option value="">Select Vendor</option>
                </select>
                <button type="button" class="btn btn-danger remove-vendor">Remove Vendor</button>
            </div>
            <button type="button" class="btn btn-secondary add-vendor">Add Vendor</button>
        `);
        $('#modulesContainer').append(newGroup);
        moduleIndex++;
    });
    ...
    $(document).on('click', '.remove-module', function () {
        if ($('.module-group').length > 1) {
            ...
            alert("At least one module group is required.");
        }
    });

    // Add Vendor inside a Module Group
    $(document).on('click', '.add-vendor', function () {
        let vendorContainer = $(this).closest('.vendor-container');
        let moduleIdx = $('#modulesContainer .module-group').index($(this).closest('.module-group'));
        let newVendorRow = `
            <div class="vendor-row d-flex mb-2">
                <select name="primary_vendor[${moduleIdx}][]" class="form-control vendor-select me-2 primary-vendor-select">
                    <option value="">Select Vendor</option>
                </select>
                <button type="button" class="btn btn-danger remove-vendor">Remove Vendor</button>
            </div>
        `;
        $(this).before(newVendorRow);
    });

    // Remove Vendor
    $(document).on('click', '.remove-vendor', function () {
        let group = $(this).closest('.vendor-container');
        if (group.find('.vendor-row').length > 1) {
