    ...
    $(document).ready(function() { 
        ...
        $('#submitBtn').click(function() {
            ...
            {   
                ...
                $.ajax({
                    url: "{{ route('license.license3') }}", // Path to controller through routes/web.php
                    type: 'POST',
                    data: { dl:dl,  dob:dob },
                    dataType: 'json',
                    success: function(response) {
						
						console.log(response.result.dlNumber);
						console.log(response.result.detailsOfDrivingLicence.photo);
                       ...
                       if(response.code == 200){
							...
							console.log(response.result.detailsOfDrivingLicence.photo);
							console.log(response);
                            $("#dlNumber").text(response.result.dlNumber);
                            $('#photo').attr('src', response.result.detailsOfDrivingLicence.photo);
                            $("#name").text(response.result.detailsOfDrivingLicence.name);
                            $("#fatherOrHusbandName").text(response.result.detailsOfDrivingLicence.fatherOrHusbandName);
                            $("#dob_r").text(response.result.dob);
                            $("#completeAddress").text(response.result.detailsOfDrivingLicence.address);
                            $("#bloodgroup").text('NA');
                            $("#state").text(response.result.detailsOfDrivingLicence.splitAddress.state[0]);
                            $("#rto").text(response.result.detailsOfDrivingLicence.dateOfIssue);
                            $("#status").text(response.result.detailsOfDrivingLicence.status);
                            $("#gender").text('NA');
                            /* var CovDetails = response.result.msg['CovDetails'];
                            var LicenseDetails = response.result.msg['LicenseDetails'];
