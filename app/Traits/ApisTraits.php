...
trait ApisTraits	
{
    ...
    public function rcSurePass($vendor, $rc_number)
    {

        $response   = '';
        $result     = [];
        $url        = Config::get('custom.surepass.rc.url');
        $authorization_key   = Config::get('custom.surepass.rc.authorization_key');
        $api_id     = Config::get('custom.surepass.rc.api_id');
        $api_name   = Config::get('custom.surepass.rc.api_name');
        $vendor     = Config::get('custom.surepass.rc.vender');

        $headers            = array("Authorization: Bearer $authorization_key",'Content-Type: application/json');
        ...
        if(isset($curlRes['status']) && $curlRes['status'] == 'success')
        {
            ...
            {                
                ...
                $error          = isset($responseData) ? $responseData : ''; 
                $statusCode     = isset($error['status_code']) ? $error['status_code'] : '101';
                $message        = isset($error['message']['errorMessage']) ? $error['message']['errorMessage'] : '';
                $result = array('status'=>'failed','status_code' => $statusCode, 'msg'=> $message, 'response'=>$response);
            }
			//Add History For 200/1 & 404/9
			if(in_array($statusCode, [200]))
			{
				$historyStatus = ApisTraits::addHistoryRCForApiList($rc_number, $jsonData, $vendor, $response, $statusCode);
			}
        }
        else{
            $result = array('status'=>'failed','status_code' => '', 'msg'=> $curlRes['msg'], 'response'=>$response);
