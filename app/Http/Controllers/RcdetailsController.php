...
class RcdetailsController extends Controller
{
    ...
    public function rcPostData(Request $request)
    {
        ...
        if ($isValidVehicleNumber === false) {
            ...
            // echo "<pre>";
            // print_r($queries); // Print the logged queries in a readable format
            // echo "</pre>";
            // exit;
            ...
            if($isCreditAvaialbe === false)
            {
                ...
                if(isset($responseHistrory) && empty($responseHistrory)) 
                {
                    ...
                    // $responseArr        = array('response'=>'','status_code'=>'','status'=> '');
	...
	public function standardRcResponse($vendor, $response)
	{
        switch (strtolower($vendor)) {
            ...
            case "sc":
                ...
                break;
            case "invincible":
                $response = $this->rcInvincibleResponsePDF($response);
                break;
            //added by pooja 28-10-2024
            case "surepass":
                $response = $this->rcSurePassResponsePDF($response);
                break;
        }
        return $response;
	}
	...
	public function rcInvincibleResponsePDF($response)
	{
		...
		if(isset($response['result']['data']) && !empty($response['result']['data'])){
			...
			$data = [
				...
				isset($msg['vehicleClass']) ? $msg['vehicleClass']." ".$msg['vehicleCategory']  : null, // Vehicle Class
    ...
    //added by pooja 28-10-2024
    public function rcSurePassResponsePDF($response){
        //echo "<pre>"; print_r($response);die;
        $returnArr = [];
        if(isset($response['data']) && !empty($response['data'])){
            $msg 	= $response['data']; 

            $header = ['Input RC Number','Vehicle Class', 'Fuel Type',	'Chassis Number',	'Engine Number',	'Manufacture Date',	'Model / Makers Class'	,'Engine Capacity',	'Color'	,'Gross Weight',	'No of cylinder'	,'Seating Capacity',	'sleeper Capacity'	,'Norms Type',	'Body Type',	'Owner Serial Number'	,'Mobile Number'	,'Unloading Weight'	,'Vehicle Standing Capacity','Blacklist Status'	,'Noc Details',	'Registration Number'	,'Registration Date',	'Fitness Date/RC Expiry Date',	'RTO',	'Tax Upto',	'Vehicle Tax Up to'	,'Status'	,'Status As On'	,'Owners Name',	'Father Name/Husband Name',	'Permanent Address'	,'Present Address',	'Financer Name'	,'Insurance To Date/Insurance Upto',	'Policy Number'	,'Insurance Company',	'PUCC NO'	,'PUCC Upto',	'Permit Issue Date',	'Permit Number',	'Permit Type',	'Permit Vald From'	,'Permit Valid Upto',	'Non Use Status'	,'Non Use From',	'Non Use To',	'National Permit Number',	'National Permit Upto',	'National Permit Issued By'];
            $data = [
                isset($msg['rc_number']) ? $msg['rc_number'] : null, // Input RC Number
                isset($msg['vehicle_category_description']) ? $msg['vehicle_category_description'] : null, // Vehicle Class
                isset($msg['fuel_type']) ? $msg['fuel_type'] : null, // fuel_type
                ...
                isset($msg['manufacturing_date']) ? $msg['manufacturing_date'] : null, // Manufacture Date
               ...
               // isset($msg['vehicleManufacturerName']) ? $msg['vehicleManufacturerName'] : null, // Maker/Manufacturer 
                ...
                isset($msg['fit_up_to']) ? $msg['fit_up_to']  : null, // Fitness Date/RC Expiry isset(Date
                ...
                isset($msg['latest_by']) ? $msg['latest_by'] : null, // Status As On
